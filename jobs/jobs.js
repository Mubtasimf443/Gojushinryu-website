...
import SocialMediaMails from "./controllars/mails/social_media_mail.js";
...
async function updateYoutube() {
    try {
        ...
        if (!status) {
            await SocialMediaMails.notConnected.youtube();
            console.error('their is no permision to post on youtube');
        }
        ...
        let time = ((new Date().getHours() < 13) ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM');
        if (data?.credentials?.access_token && data.credentials?.refresh_token) {
            ...
            console.log('youtube token updated at :' + time)
            return;
        ...
        try {
            ...
            settings.youtube_refresh_token = null;
            settings.youtube_access_token_status = false;
            settings.youtube_token = null;
            await settings.save();
            await SocialMediaMails.notConnected.youtube();
        } catch (error) {
            console.error(error);
...
async function updateTiktok(params) {
    try {
        let { access_token, refresh_token } = await Settings.findOne({}).then(function (set) {
            ...
            return { refresh_token: set.tiktok_refresh_token, access_token: set.tiktok_access_token }
        });
        ...
        if (data.message === 'success') {
            ...
            if (data.access_token && data.refresh_token) {
                ...
                await set.save();
                let time = (new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM');
                console.log('tiktok tokens updated at ' + time);
                return;
            }
        }
        ...
        try {
            ...
            await set.save();
            let time = (new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM');
            await SocialMediaMails.notConnected.titkok()
            log(`tiktok tokens removed at ${time}`);
        } catch (error) {
            console.error(error);
        }
    }
...
async function updateLinkedin(params) {
    try {
        ...
        let time = new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12);
        await Settings.findOneAndUpdate({}, {
            linkedin_access_token_status: true,
            linkedin_access_token: access_token,
            linkedin_refresh_token: refresh_token
        })
        console.log('linkedin data updated at ' + time + ' PM');
        ...
        try {
            ...
            await SocialMediaMails.notConnected.LinkedIn();
            let time = (new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM')
            console.log('linkedin data deleted at' + time);
        } catch (error) {
            console.error(error);
        }
    }
...
async function updateFacebook() {
    try {
        ...
        let time = (new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM');
        console.log('facebook tokens updated at ' + time);
        ...
        try {
            ...
            let time = (new Date().getHours() < 13 ? new Date().getHours() + ' AM' : (new Date().getHours() - 12) + ' PM');
            await SocialMediaMails.notConnected.facebook();
            await SocialMediaMails.notConnected.Instagram();
            console.log('facebook tokens and data are removed at ' + time);
        } catch (error) {
            console.error(error);
        }
    }
